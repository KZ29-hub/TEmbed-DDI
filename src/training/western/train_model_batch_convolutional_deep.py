...
class MainModel(nn.Module):
    ...
    def forward(self, data):
        ...
        embeddings = self.last_token_pool(output_desc1.last_hidden_state, encoded_desc1['attention_mask'])
        # desc_embedding1 = F.normalize(embeddings, p=2, dim=1)
        embeddings2 = self.last_token_pool(output_desc2.last_hidden_state, encoded_desc2['attention_mask'])
        # desc_embedding2 = F.normalize(embeddings2, p=2, dim=1)

        x = torch.cat((embeddings, embeddings2), dim=1)
...
# Training example
### weight_decay=1e-5
def train(model, data_list, labels, test_data_list, test_labels, csv_file, epochs=10, batch_size=32, lr=2e-5, dataset_name='unknown', num=0):
    ...
    for epoch in range(epochs):
        ...
        # print('lr', lr)
        ### 选取随机固定数量
        indices = np.random.choice(len(data_list), num, replace=False)
        selected_train_json = [data_list[i] for i in indices]
        selected_train_true_labels = [labels[i] for i in indices]

        print(f"Epoch {epoch}/{epochs}: Training on {num} samples")
        
        model.train()
        total_loss = 0
        for i in tqdm(range(0, len(selected_train_json), batch_size), total=len(selected_train_json)//batch_size,desc=f'{epoch} training'):
            batch_data = selected_train_json[i:i + batch_size]
            batch_labels = selected_train_true_labels[i:i + batch_size]
        ...
        average_loss = total_loss / (len(selected_train_json) // batch_size)
...
if __name__ == '__main__':
    ...
    train(model, train_json, train_true_labels, test_json, test_true_labels, args.csv_file, args.epoch, args.batch_size, args.lr, args.dataset_name, num)
