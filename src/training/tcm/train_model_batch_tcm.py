...
class MainModel(nn.Module):
    ...
    def forward(self, data):
        # Extract data
        desc1 = [data[i]['tcm1_name'] + data[i]['tcm1_desc'] for i in range(len(data))]
        desc2 = [data[i]['tcm2_name'] + data[i]['tcm2_desc'] for i in range(len(data))]
...
def train(model, data_list, labels, test_data_list, test_labels, csv_file, epochs=10, batch_size=32, lr=2e-5, dataset_name='unknown'):
    ...
    for epoch in range(epochs):
        ...
        if epoch in [0, 4, 9, 14, 19, 24, 29, 34, 39, 44, 49] or epoch == epochs - 1:  # Test at 10, 20, 30 and last epoch
...
def test(model, test_data_list, test_labels, batch_size=32):
    ...
    print(confusion_matrix(true_labels, predictions2))
...
if __name__ == '__main__':
    ...
    train_path = "/root/autodl-tmp/multiview-project-1023/embeddings/western_data/chch_ddi_fold0/train_balanced.jsonl"
    test_path = "/root/autodl-tmp/multiview-project-1023/embeddings/western_data/chch_ddi_fold0/test_balanced.jsonl"
